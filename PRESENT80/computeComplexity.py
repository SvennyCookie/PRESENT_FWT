...
def get_red_str(k_cross,k_green):
    ...
    k_16_cross = minus_16(k_cross)
    k_16_green = minus_16(k_green)

    k_16_cross_str = '['
    for i in range(len(k_16_cross)):
        if (k_16_cross[i] in k_16_green):
            k_16_cross_str += '\\red{'+str(k_16_cross[i])+'}, '
        else:
            k_16_cross_str += str(k_16_cross[i])+', '
    k_16_cross_str = k_16_cross_str[0:(len(k_16_cross_str)-2)]
    k_16_cross_str += ']'
    return k_16_cross_str

mask_list =  [[4, 2, 5, 13], [4, 2, 5, 15], [4, 2, 9, 13], [4, 2, 9, 15],
              [4, 8, 5, 5], [4, 8, 5, 7], [4, 8, 9, 5], [4, 8, 9, 7],
              [4, 4, 5, 13], [4, 4, 5, 15], [4, 4, 9, 13], [4, 4, 9, 15],
              [4, 2, 13, 13], [4, 2, 13, 15], [4, 8, 13, 5], [4, 8, 13, 7]]
...
for item in mask_list:
    ...
    if (m == 8):
        ...
        ti = [64, 64]
        si = [64, 64]
        Li = [((10/16)**15)*(2**si[0]), ((10/16)**15)*(2**si[1])]
    ...
    tmp_str = "$k_{1}$: "+str(minus_16(k1_deduce_from_ki))    
    str_deduced_bits_from_ki = tmp_str+"$, k_{30}$: "+ str(minus_16(k30_deduce_from_ki))
    ...
    if (m == 2):     
        ...
        for index in range(16):
            V1.append(index*4 + 16)
            V1.append(index*4 + 17)
            V1.append(index*4 + 19)

            V2.append(index*4 + 16)
            V2.append(index*4 + 18)
            V2.append(index*4 + 19)
    ...
    if (m == 4):
        ...
        for index in range(16):
            V1.append(index*4 + 16)
            V1.append(index*4 + 17)
            V1.append(index*4 + 18)

            V2.append(index*4 + 16)
            V2.append(index*4 + 17)
            V2.append(index*4 + 19)
        W_perp = k30_deduce_from_ki
...
print("2^", np.log2(float(sum(add_time))), 'times additions')

print("2^", np.log2(float(sum(multi_time))), 'times multiplications')
